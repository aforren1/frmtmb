...
The long format is the supported spelling for a multi-trait model with
a known covariance. Do not reach for `mvbf()` with the `|ID|`
identifier: frmtmb refuses that combination, because the cross-formula
merge cannot carry a `gr(cov = )` matrix through.
...
- A multi-trait model with a known covariance takes the long format of
  section 1. `mvbf()` with the `|ID|` identifier over a `gr(cov = )`
  block is refused, because the cross-formula merge cannot carry the
