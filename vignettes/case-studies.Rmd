...
them. An `ar()`, `ma()`, `arma()`, `cosy()` or `unstr()` term next to
`rescor = TRUE` is refused, and so are mixtures and censored
responses. One `|ID|` label must name one grouping specification: the
same label over two different grouping factors is an error, because
there is no single block for it to build. `residuals()` and `fitted()`
are not the only parts of the post-fitting surface that step back from
a `rescor = TRUE` fit. `frm_compat("rescor")` lists the whole set, and
`vignette("compatibility")` says why the list is worth reading before
the fit rather than after it.
