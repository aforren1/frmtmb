...
has_spline <- requireNamespace("frmtmb.spline", quietly = TRUE)
```
...
`predict()` takes `resp`, so one response at a time is available, and
a residual is the subtraction. The figure below needs both levels:
...
functions off `predict()`: `b0(t)` is the prediction at `x = 0` and
...
out. Draw the coefficient function from `predict()` as above.
...
`vignette("ddm", package = "frmtmb.ddm")` is the worked model, and
`frmtmb.ddm::wiener()` is the family.
...
at the end how many rows fell outside it, and `predict()` reports it
again for a grid that leaves the span.
...
needs, and `frmtmb.spline::frm_curve()` builds one from the same two
objects. It refused a nonlinear body until this seam existed.

```{r smocc-simultaneous, eval = has_brokenstick && has_spline}
...
- On a model with an `fs` factor-smooth, `predict(re.form = NA)`
...
- `residuals()` and `fitted()` refuse a multivariate fit. `predict()`
...
- `vignette("latent", package = "frmtmb.latent")` fits a hidden Markov
  ...
  the rows refuses.
- `vignette("ddm", package = "frmtmb.ddm")` fits the drift-diffusion
