...
test_that("every condition message template in this R/ is unique", {
  ...
  # than asserted per kind: this family refuses rather than narrates, so
  # it raises no message() and no warning() at all, and a per-kind
  # assertion would fail on a true statement about the package. The sum
  # still guards against a collect() that silently matches nothing,
