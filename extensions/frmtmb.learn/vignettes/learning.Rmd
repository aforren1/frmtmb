...
and more. The other refusal worth knowing is the Laplace correction:

```{r refuse-importance, error = TRUE}
...
* `?bandit2arm_delta` and the other five family pages for the equations
  and each family's own cautions.
* `?frm_value_trace` for the trajectory, and `?frm_task_simulate` for
  drawing datasets from the generative process.
* `?frmtmb.learn` for the Laplace numbers and the protocol seam that
  `frm(importance =)` waits on.
