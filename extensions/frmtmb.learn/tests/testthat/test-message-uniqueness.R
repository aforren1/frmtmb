...
#
# That distinction earns its keep here more than in most packages. Six
# families share one set of refusals, and each refusal is written ONCE
# with the family name filled in at run time, so a user of any of the
# six reads a sentence in their own family's name and a maintainer
# reading a bug report finds one line of source.
#
...
test_that("every condition message template in this R/ is unique", {
  ...
  is_src <- file.exists(desc) &&
    any(trimws(readLines(desc, n = 5L)) == "Package: frmtmb.learn") &&
    dir.exists(rdir) &&
    file.exists(file.path(rdir, "engine.R"))
  skip_if_not(is_src,
  ...
  # the parser's own sanity check, summed over the three kinds rather
  # than asserted per kind: this package refuses rather than narrates,
  # so it raises no message() and no warning() at all, and a per-kind
  # assertion would fail on a true statement about the package. The
  # refusals are the whole surface here, because every one of them
  # names a thing the construction cannot do rather than a thing that
  # went wrong. The sum still guards against a collect() that silently
  # matches nothing, which is what it is for.
  expect_gt(found, 0)
})
