...
#' records the received outcome alone can pass it twice: the fit is then
#' EXACTLY right rather than nearly so. The unchosen entry is
#' multiplied by a zero indicator before it reaches anything, so the
#' objective is bitwise the same function of the parameters whatever
#' that column holds.
#'
#' A DRAW is a different question. A simulated subject chooses for
#' itself, so paying it needs what the arm the real subject did not take
#' would have given, which a duplicated column does not record. With the
#' two columns identical on every trial the drawn choices carry no
#' learning signal at all, so `simulate()`, `frm_simulate()` and
#' [frm_task_simulate()] refuse that data by name rather than returning
#' a draw from a task nobody ran. Some rows equal is not the signature;
#' every row is. There is no way round it through the data:
#' `simulate()` refuses `newdata` for a learning family, whose draw walks
#' the fitted trial sequence, and any other data set drawn through this
#' model is read by a formula that names one column twice. Refit with a
#' column per arm.
#'
...
#' session applies in the next. `session =` says so, and every family
#' in this package takes it:
#'
#' ```
...
#' initial values. Trial numbers need to be unique only within a
#' session, so a count that starts again at 1 each day needs no
#' renumbering. The subject is still the unit: its sessions share its
#' random effects, and `frm(importance =)` resamples it whole, with all
#' of its sessions. [frm_value_trace()] gains a `session` column.
#'
...
#' comes back after another session, as in `a b a b`, is refused by
#' name: one session over its two runs would carry the value store
#' across the trials between them, which is neither a restart nor a
#' continuous sequence. Interleaved contexts, such as two tasks that
#' alternate within a day with a value store each, are a different
#' model: one store per context, kept across that context's runs. That
#' model is not built; it is filed as a possible later extension.
#'
...
#' replicates of 40 subjects by two sessions of 100 trials, each
#' session drawn from a fresh store, with `alpha ~ 1 + (1 | id)` at a
#' between-subject standard deviation of 0.5 and `tau` = 3:
#'
#' \tabular{lrrrr}{
#'   \tab truth \tab mean, with \tab covered, with \tab covered, without \cr
#'   logit alpha \tab -0.619 \tab -0.605 \tab 94.6 \tab 91.1 \cr
#'   log tau \tab 1.099 \tab 1.099 \tab 95.0 \tab 74.8 \cr
#'   log sd(alpha | id) \tab -0.693 \tab -0.763 \tab 97.5 \tab 97.5
#' }
#'
#' The last two columns are percent coverage of the Wald interval, and
#' every fit converged. With
#' `session =` no interval's Wilson bound excludes 95 percent. Without
#' it the choice sensitivity comes back 2.7 percent low and its
#' interval covers three times in four. The log standard deviation
#' comes back 0.070 low with `session =`, 4.9 Monte Carlo standard
#' errors, and its interval still covers at 97.5 percent; this study
#' does not separate the Laplace caveat below from ordinary maximum
#' likelihood shrinkage as its cause. `dev/phase3b-findings.md` has the
#' construction.
#'
...
#'
#' @section A correlated block on both parameters, measured:
#' `(1 | p | id)` on the learning rate AND the inverse temperature is
#' this family's whole parameter vector, and it recovers at 100 learners
#' by 200 trials: over 60 replicates the two standard deviations come
#' back at 0.4925 and 0.2980 against 0.5 and 0.3, and the correlation at
#' 0.5322 against 0.5, with coverage 0.92 to 0.97. The same design with
#' a correlation of ZERO returns 0.0214, so the estimator does not
#' invent one. `?frmtmb.learn` has the tables and
#' `dev/learnhier-findings.md` the construction.
#'
#' Read a correlation's INTERVAL rather than its point estimate. The
#' estimates spread by 0.140 across those replicates and range from
#' 0.172 to 0.804 for a truth of 0.5, so one dataset of this size
#' locates a correlation to about half a unit.
#'
...
#' @export
bandit2arm_delta <- function(subject, trial = NULL, session = NULL) {
  spec <- ln_spec(
    n_option = 2L,
  ...
  ln_family("bandit2arm_delta", session_expr = substitute(session),
