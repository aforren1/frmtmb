...
# the package loads must sit in this unconditional chunk: the tangled
# script skips eval-gated chunks, so a library() inside one leaves every
# later unconditional chunk without frm()
library(frmtmb)
library(frmtmb.eam)
...
```{r fig-bound, eval = has_tinyplot, fig.width = 7, fig.height = 4, fig.alt = "Plot of the boundary against time in the trial, from 0 to 2 seconds. Three mirrored pairs of lines all start at plus and minus 1.5. The gray constant pair stays flat. The blue exponential pair closes fastest and reaches about plus and minus 0.3 at 2 seconds. The red linear pair closes steadily to about plus and minus 0.6. A dotted line marks zero, where the accumulator starts."}
tmax <- 2
...
The closed form above is only a check on the part of this family that
is not generalized. With a constant drift and fixed bounds the model is
the Wiener model and the solver can be held to the Navarro-Fuss
density; nothing in that comparison reaches the leak, the collapsing
bounds, the coherence nonlinearity, the start-point variability or the
lapse, which are the reasons to use this family at all. Gradient tests
show the derivative matches the value it is the derivative of, which is
equally true of a wrong density, and parameter recovery is circular
here on purpose, because `gddm_simulate()` draws from the solver's own
density.
...
and `tests/testthat/test-gddm-reference.R` reads it. No Python runs
when the tests run; the fixture is regenerated by hand with
...
uniform start-point variability; and a uniform lapse. For each of
these, three things: the defective density at both boundaries on a
common time grid, the two boundary masses, and the log-likelihood of a
small fixed dataset whose response times fall between grid nodes, so
that the likelihood's own interpolation is exercised. At the shipped
grid the two agree to between 7e-4 and 1.6e-3 in the log density on
every case but one, which is an order of magnitude better than the 0.01
claimed above, and the test also requires the disagreement to shrink as
`dt` and `ny` are refined, so the reference is what the solver
converges to rather than merely something it lands near.
...
translated rather than tested: this family renormalizes each
condition's pair of densities so that its likelihood is conditional on
a response inside the window, while PyDDM keeps the undecided mass; and
this family spreads a lapse over `t_max` where PyDDM spreads it over
one time step more.
...
no Crank-Nicolson path for a boundary that moves, so it solves that
case with backward Euler on a grid-snapped bound, first order in both
steps, and its own recorded convergence residual is 2.6e-3. Solved one
level finer and extrapolated to zero step, PyDDM moves toward this
package: measured against that limit rather than against the frozen
fixture, `gddm()` is 8.1e-4 out at the shipped grid, then 2.8e-4, then
1.7e-4 as its own grid is refined. 8.1e-4 is inside the band the other
nine cases occupy, so there is no floor here and nothing about this
case that `gddm()` gets wrong.

The same is true of the start-variability case, which looks as though
it settles at 2.6e-4. `ICRange` lays the uniform start on whole grid
cells, so PyDDM's box is about one cell too wide, an error first order
in its spatial step. Against PyDDM's extrapolated limit `gddm()` runs
7.2e-4, 2.1e-4, 6.5e-5 over the same three grids, which is the ordinary
second-order sequence. In both cases the mechanism is a first-order
error in the reference, and in both `gddm()` is the more accurate of
the two: holding the walls still, and spreading the start with an
integrated cubic B-spline instead of snapping it to cells, is what buys
that.

**A third opinion.** Both packages march a partial differential
equation on a grid, so agreeing with each other does not rule out an
error they make in the same direction. The test also simulates the
stochastic differential equation directly by Euler-Maruyama on the
collapsing-bound case, where a shared error would be least surprising.
A simulated first passage is late, because absorption is only noticed
at a monitoring time, so two monitoring gaps driven by the same
Brownian path are extrapolated to zero gap. It agrees with both
packages to one or two standard errors, and it resolves a shared bias
down to roughly two parts in a thousand. It is the size of those gaps
that carries the conclusion and not which side of the reference a run
falls on: the two quantities compared are anti-correlated by
construction, so landing on opposite sides is the null expectation
rather than evidence, and the observed sides change with the seed.
...
```{r fig-grid, eval = has_tinyplot, fig.width = 7, fig.height = 4, fig.alt = "Plot of the fitted upper-wall density on a logarithmic axis from 1e-13 to 20, against response time from 0.2 to 0.6 seconds, for three time steps. A dashed vertical line marks the fitted non-decision time near 0.24 seconds. The three curves lie on one another from about 0.4 seconds on, where the density is near 5. To the left of that they separate over many orders of magnitude: just past the dashed line the dt 0.04 curve is near 1e-3, the dt 0.02 curve near 1e-5 and the dt 0.01 curve near 1e-8."}
...
The result behind the Euler-Maruyama extrapolation: a first passage
noticed only at a monitoring time is late by an amount of the order of
the square root of the monitoring gap, not of the gap.
