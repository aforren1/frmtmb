...
test_that("every condition message template in this R/ is unique", {
  ...
  is_src <- file.exists(desc) &&
    any(trimws(readLines(desc, n = 5L)) == "Package: frmtmb.latent") &&
    dir.exists(rdir) &&
    file.exists(file.path(rdir, "hmm.R"))
  skip_if_not(is_src,
  ...
  # the parser's own sanity check, and the one place this differs from
  # frmtmb's copy: there it is asserted per KIND, because core raises
  # all three. Both families here refuse rather than narrate, so they
  # raise no message() at all and a per-kind assertion would fail on a
  # true statement about the package. Summing keeps the guard against a
