...
`re_formula = NA` drops the per-subject curves, so what comes back is the
POPULATION curve.
...
it is small: at `re_formula = NA` the per-subject coefficients contribute
...
Drop `re_formula = NA` and supply the grouping column, and the same three
