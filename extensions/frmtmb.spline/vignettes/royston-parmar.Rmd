...
`frm_linpred(type = "link", dpar = )` reaches each of them and the fitted
...
A RANDOM effect can go on either coefficient, and the two mean
different things. `(1 | centre)` on `mu` is a shared log-normal
frailty, the model `rstpm2::stpm2(cluster =, RandDist = "LogN")` fits.
...
`?royston_parmar`, section "A frailty, and a random effect on gamma1",
for what each one estimates and what each one was measured to recover.

## What this family will not do

`fitted()` and `frm_linpred(type = "response")` refuse, and say why. The
...
silent in the fitted object. `rp_floored()` is where it goes to be
read. It refuses on that one and reports on a second quantity that used
to be a floor and is not one any more:

```{r floored}
rp_floored(fit, action = "report")
```

Zero on each count here. Run it on every fit whose data carry
...
floor is gone. `n_censored_deep` still counts the rows that used to
fall in it, because a censored row whose fitted survival probability is
...
That fit is now SCORED CORRECTLY and `rp_floored()` does not refuse it.
The one deep row is counted, at `-log S` of 55.73, and its contribution
is `-55.7302136` exactly where the old form gave `-Inf` and the floor
gave -35.127363. The reported log likelihood reproduces an independent
one written from the hazard-scale definition to 4.2e-12.

What has NOT changed is that `logLik()` and `AIC()` cannot be gated from
an extension. `logLik()` reads `object$opt$objective`, one number, and
the protocol has no per-row or per-group log-likelihood slot through
which a family could say which rows carried it. `post$fit_check`,
frmtmb's fit-end hook, is what this family uses instead: it warns as the
fit is returned. That covers the monotonicity floor, which is a property
of the fitted parameters. It cannot cover a row-level accounting that
the protocol does not expose.

Monotonicity of the cumulative hazard is the floor that remains, and it
is not enforced; `flexsurv` does not enforce it either, and a spline that turns
...
pseudo-likelihood. `rp_floored()` counts those rows and refuses on
them, and `post$fit_check` warns about them as the fit is returned. You
...
not positive is still a defect, of the MODEL rather than of `logLik()`:
the fitted survival function rises with time there. It happens when a
group has its own slope on `gamma1` and no events, because the
`log(gamma1 + u)` barrier that keeps a slope positive lives in the
density and that group contributes none. The same happens past an
arm's last event in a cure-fraction design with `gamma1 ~ arm`.
`rp_floored()` counts those rows apart, as `n_nonmonotone_censored`,
and WARNS on them with the size of the rise, since `logLik()` is still
the model's; it refuses only on the event rows.
