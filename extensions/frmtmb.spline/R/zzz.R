...
sp_compat_rules <- function() {
  ...
  r("royston_parmar", "us", "works",
    "A (1 | centre) block on mu is a shared log-normal frailty: mu is gamma0, so the centre deviation adds to the whole log cumulative hazard and multiplies the cumulative hazard by exp(b). Checked against rstpm2's own log-normal frailty, stpm2(cluster =, RandDist = \"LogN\"), which is the same model in another basis: the change of basis from nsx() coordinates to Royston and Parmar's is exact to 3.0e-14 of the linear predictor's scale. Over 200 replicates at 2000 subjects in 40 centres, 40 percent censored, the treatment coefficient agrees to 3.2e-04 of the run's own standard error on average and 8.8e-04 at worst, the frailty standard deviation to 1.4e-03 relative, and the standard errors on that coefficient to 2.6e-06 relative. The two report DIFFERENT log likelihoods and the difference is the integration rule, not the model: frmtmb's Laplace value is 0.058 units under an exact per-cluster integral where rstpm2's 9-node adaptive Gauss-Hermite is 4.9e-06 under it. That offset costs the ESTIMATE 5.5e-05 units of exact log likelihood, which is 0.1 percent of the offset itself, and it grows as the cluster shrinks, reaching 1.38 units at 4 subjects per centre. A block on gamma1 or higher is a per-centre TIME-VARYING effect and not a frailty; ?royston_parmar says what it means and what it recovers.")
  r("royston_parmar", "|ID|", "works",
    "(1 | c | centre) on mu and gamma1 together fits a correlated centre intercept and centre slope in log time, and it is the spelling that makes a random effect on gamma1 mean the same thing in every time unit. A block on gamma1 ALONE is anchored at t = 1, because that is where log t is zero, and a time unit is not a fact about the data. Measured on one 2000-row dataset read in years, months and days, comparing log likelihoods as logLik + n_event log c, which is what a density owes a rescale: the paired block moves by 4.3e-08 units and holds sd(gamma1 | centre) at 0.2259 in all three, while the slope-only block moves by 8.19 units and its sd(gamma1 | centre) falls from 0.2201 to 0.0370.")
  ...
  ## ---- curve inference --------------------------------------------
  r("frm_curve", "s()", "works",
    "The case this package is for. The penalized smooth's wiggly part is a random-effect block, so the curve covariance needs the joint covariance of the fixed and random coefficients; frm_curve() assembles it and checks it against frm_linpred(se.fit = TRUE) on every call.")
  ...
  r("frm_curve", "rr", "works",
    "Both re_formula settings, since this package reads frmtmb::frm_lp_basis() instead of rebuilding the design. A reduced-rank block's loadings live in theta, so a design over (beta, b) alone is INCOMPLETE; the unit-perturbation rebuild this package used could not see the derivative with respect to the loadings and its standard errors came out 27 percent away from frm_linpred(se.fit = TRUE)'s at re_formula = NULL, which the covariance check refused. frm_lp_basis() carries the loading columns through rr_jacobians(). Measured on a 5-trait rr(d = 2) fit over 60 groups: agreement 0.0e+00 at re_formula = NA (5 columns) and at re_formula = NULL (134 columns, 9 of them theta).")
  r("frm_curve", "predict", "works",
    "frm_curve() is built out of predict(): the design is the difference between predictions one coefficient apart, and the answer is checked against frm_linpred(se.fit = TRUE).")
  r("frm_curve", "autoscale", "works",
    "Measured, and it is a consequence of reading core's cached covariance rather than recomputing one. An autoscaled fit carries par_units and its covariance comes from autoscale_sdreport(), which reparameterizes; a fresh RTMB::sdreport() would not, and this package used to call one. On y ~ s(x, k = 10) + z with z at scale 1e6, par_units spans 9.7e-07 to 1 and frm_curve() agrees with frm_linpred(se.fit = TRUE) to 3.7e-16 absolute, cov_rel_error 6.7e-15.")
  ...
  r("frm_curve", "nl", "works",
    "The consumer dev/spline-seam-proposal.md was written for. A nonlinear body is not linear in its coefficients, so the linearity probe this package used to run refused it; frm_lp_basis() tapes the body instead and returns d eta / d coef as a Jacobian. frm_linpred(se.fit = TRUE) is still refused for a nonlinear predictor, so there is NO second route to check against: cov_rel_error comes back NA and print() says the check did not run. Measured on the warped-growth model of D'Alessandro, Thoresen and Sorensen (2026) fitted to brokenstick::smocc_200, and on a smaller ps() fit whose Jacobian agrees with a central difference of predict() to 1e-5.")
  ...
  r("frm_curve_contrast", "nl", "works",
    "Measured on a two-ps() nonlinear body, y ~ lev + ps(t) + b1 w + b2 w t + ps(z), differencing across w. frm_lp_basis() tapes the body and returns a Jacobian per grid, and the difference of two Jacobians is the Jacobian of the difference because the subtraction is linear. frm_linpred(se.fit = TRUE) is refused for a nonlinear predictor, so cov_rel_error is NA here for the same reason it is NA for one curve, and print() says so.")
  ...
  r("frm_curve_contrast", "predict", "works",
    "Two calls rather than one, one per grid, and both are the covariance check. What frm_linpred(se.fit = TRUE) cannot supply is the covariance BETWEEN the grids, which is the whole content of a difference, so the check licenses the two designs and not the difference's own standard error.")
