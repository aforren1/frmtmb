...
sp_compat_rules <- function() {
  ...
  r("frm_curve", "rr", "works",
    "Both re_formula settings, since this package reads frmtmb::frm_lp_basis() instead of rebuilding the design. A reduced-rank block's loadings live in theta, so a design over (beta, b) alone is INCOMPLETE; the unit-perturbation rebuild this package used could not see the derivative with respect to the loadings and its standard errors came out 27 percent away from predict(se.fit = TRUE)'s at re_formula = NULL, which the covariance check refused. frm_lp_basis() carries the loading columns through rr_jacobians(). Measured on a 5-trait rr(d = 2) fit over 60 groups: agreement 0.0e+00 at re_formula = NA (5 columns) and at re_formula = NULL (134 columns, 9 of them theta).")
