...
[`frm_ode()`](https://aforren1.github.io/frmtmb/frmtmb.ode/reference/frm_ode.md)
is in **frmtmb.ode**, a package of its own that adds the solver to
frmtmb’s formula grammar. Both live in the same repository:
...
It needs the **RTMBode** package in turn, which supplies a
differentiable interface to the **deSolve** integrators. RTMBode is not
on CRAN:
...
Everything below is skipped if RTMBode is not installed.
