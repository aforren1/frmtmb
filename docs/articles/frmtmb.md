...
fixef(nlfit, flatten = TRUE)
#>     a_(Intercept)     b_(Intercept) sigma_(Intercept) 
#>         2.5067769         0.7022003        -1.9048113
```

Every nonlinear parameter starts at zero unless `start` or a located
prior moves it. For a monotone term that is usually good enough. For a
BUMP-SHAPED term it is not, because a bump evaluated away from the data
is flat in every one of its parameters at once. Write a peak
`exp(lamp) * exp(-0.5 * ((w - pk) / exp(lsig))^2)` over `w` from 1 to 45
and `pk` starts at 0, where the bump underflows to zero everywhere: the
amplitude, the centre and the width then have no gradient, the Hessian
is singular and every standard error is `NaN`. The fit is not
overparameterized. It never had a gradient to follow.

Two spellings avoid it. Put the centre where the data is, either with a
start value or by writing the offset into the formula:

``` r

# start the centre in the middle of the range
frm(bf(I ~ exp(lamp) * exp(-0.5 * ((w - pk) / exp(lsig))^2),
       pk ~ 1, lamp ~ 1, lsig ~ 1, nl = TRUE),
    family = exponential("log"), data = d,
    start = list(beta = c("pk_(Intercept)" = 10)))

# or estimate a DEPARTURE from a centre you already know
bf(I ~ exp(lamp) * exp(-0.5 * ((w - (10 + pk)) / exp(lsig))^2), ...)
```

`diagnose(fit)` names the flat parameters when this happens, under “Flat
directions”.

A nonlinear parameter also cannot be named after one of the family’s own
distributional parameters. `mu ~ 1 + (1 | id)` in a `nl = TRUE` model
gives the family’s own `mu` a linear predictor, and the model body then
has no `mu` to refer to;
[`frm()`](https://aforren1.github.io/frmtmb/reference/frm.md) refuses it
and names the reserved set. Any other name is free, including `b` and
`theta`. Those two are also parameter-template component names, so
`start = list(b = )` still means the random-effect vector, not the
parameter: a nonlinear parameter’s coefficients live inside `beta`, as
...
An inverse link saturates, so a density written over `1 - mu` or
`log(mu)` loses the tail: `plogis(eta)` is exactly 1 in double precision
from `eta = 36.7368` up, and there `1 - mu` is exactly 0, which gives
`NaN` for the value and for the gradient.
[`frm()`](https://aforren1.github.io/frmtmb/reference/frm.md) keeps each
linear predictor beside its distributional parameter while the objective
is taped, and
[`?"frmtmb-robust-dpars"`](https://aforren1.github.io/frmtmb/reference/frmtmb-robust-dpars.md)
is how a density reads it.

``` r

# a gate the optimizer has pushed against 1
d_on <- list(zi = plogis(40), .eta_zi = 40)
c(plain = log(1 - d_on$zi),
  robust = dpar_log1m(d_on, "zi", "logit"))
#>  plain robust 
#>   -Inf    -40
```

[`check_custom_family()`](https://aforren1.github.io/frmtmb/reference/check_custom_family.md)
supplies no linear predictors, so it exercises the fallback path only.
Test the other one by building the `dpars` list by hand, as above.
