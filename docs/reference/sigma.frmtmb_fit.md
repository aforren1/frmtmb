...
exist; the method warns and returns `NA` (use
`frm_linpred(dpar = "sigma")` for the per-observation values). Families
without a `sigma` parameter return 1, following glmmTMB.
