...
columns. Use [`fixef()`](fixef.md) for the fixed effects alone.
...
layer keyed like [`fixef()`](fixef.md) is added. Smooth terms are
excluded. A fit without random effects returns [`fixef()`](fixef.md)
(the single coefficient vector when there is one linear predictor).
