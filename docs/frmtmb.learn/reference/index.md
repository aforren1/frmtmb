...
One recursion engine, six models. Each parameter is an ordinary
...
random effect the way a mean does.
...
These families declare no mean on the response scale, because the
response is a nominal option code, so fitted() and residuals() refuse.
The trajectory is read here instead.
