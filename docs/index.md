...
The second need is migration. brms code ports by removing the priors and
changing `brm()` to
...
calls through that transform unchanged. A user can therefore screen
models at millisecond speed, then return to brms for the final Bayesian
fit with the same formula.
...
Pre-release (v0.39). The goal is a CRAN release. Validation has three
layers:
