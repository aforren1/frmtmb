...
- [`hmm()`](https://aforren1.github.io/frmtmb/frmtmb.latent/reference/hmm.md)
  marks its transition parameters `tr<i><j>` as read on the link scale
  (a new family field, `link_scale_dpars`), so frmtmb 0.59.0 reports
  them as coefficients `b_tr12_Intercept` rather than under a
  probability-like name holding a logit.
