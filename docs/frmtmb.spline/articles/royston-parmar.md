...
Royston-Parmar model writes the log cumulative hazard as a natural cubic
spline in log time. You get proportional hazards when you want them, a
smooth estimated baseline you can plot, and time-varying effects when
...
as an observed event and `1` as right censored. A `Surv()` status column
is the other way round. Convert it, once, and name the column for what
it holds:
...
near 800. Note `flexsurv`’s `k` is this package’s `df - 1`: `k` counts
the interior knots and `df` counts them plus one.
