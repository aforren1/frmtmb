...
`window = "hann"` is the fourth option and the one that buys nothing.
Use it where leakage is the larger error, which is a steep spectrum: a
raised cosine leaves the draw count alone (8.02 measured against a
nominal 8) and cannot be combined with `tapers` or `smooth`, for the
reason above.

What a window and a taper both cost is the independence of the rows,
which `n` does not describe. At the default eight segments the
correlation of adjacent frequencies is 0.40 with a Hann window and -0.01
without, and 0.75 at `tapers = 4`. As a variance inflation factor for a
mean over the rows that is 1.8 for the window, 4.1 for four tapers and
8.0 for eight, so an `s(freq)` band fitted on a tapered frame is
optimistic. The per-row `n` is right in all of them; it is a different
quantity.

A recording with artifact-rejected spans in it is one call. `NA` marks a
sample the pair does not have; the record is cut there and segments are
laid inside the clean spans, so no transform crosses a gap and the
answer is exactly the sum of the clean pieces. `n` then reports the
