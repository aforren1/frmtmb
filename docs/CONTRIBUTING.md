...
- **Reference agreement.** Fit the same model with an exact reference
  ...
  explicit tolerance. This is the strongest test and is preferred. The
  drmTMB tier is gated on `FRMTMB_DRMTMB_FIT_TESTS`, because every test
  in it fits about 50 models; see
