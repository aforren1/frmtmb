...
3.  **Tighten the sampler.**
    ...
    Raising `adapt_delta` shortens the step size the adaptation settles
    on, which is what removes the divergences a residual funnel
    produces, and the cost is wall-clock time. `max_treedepth = 12`
    answers the companion warning. Read the step size back with
    `nuts_params(ds)` to see what the change bought. The fit-time
    options of
    [`frmtmb_control()`](https://aforren1.github.io/frmtmb/reference/frmtmb_control.html)
    are `fit_control` in this function.
