...
gd_solve <- frmtmb.eam:::gd_solve
gd_shift <- frmtmb.eam:::gd_shift
...
ddm_lpdf_both <- frmtmb.eam:::ddm_lpdf_both
...
## Checked against PyDDM

The closed form above is only a check on the part of this family that is
not generalized. With a constant drift and fixed bounds the model is the
Wiener model and the solver can be held to the Navarro-Fuss density;
nothing in that comparison reaches the leak, the collapsing bounds, the
coherence nonlinearity, the start-point variability or the lapse, which
are the reasons to use this family at all. Gradient tests show the
derivative matches the value it is the derivative of, which is equally
true of a wrong density, and parameter recovery is circular here on
purpose, because
[`gddm_simulate()`](https://aforren1.github.io/frmtmb/frmtmb.eam/reference/gddm_simulate.md)
draws from the solver’s own density.

So the generalized components are checked against PyDDM, the reference
implementation of the model in Shinn et al. (2020). PyDDM discretizes
the same equation in an unrelated way: a fixed grid in the original
coordinate with the moving boundary sandwiched between the two nodes
around it, where this package changes variable so that the boundary is
pinned and the grid never moves. Ten cases are frozen into a fixture,
and `tests/testthat/test-gddm-reference.R` reads it. No Python runs when
the tests run; the fixture is regenerated by hand with
`dev/gddm-pyddm-reference.py` in the source repository, and the test
asserts its metadata so that a regenerated fixture cannot quietly
redefine what is correct.

**What the reference covers.** A constant drift with fixed bounds, as
calibration; a biased point start with a long non-decision time; leaky
and unstable integration, one for each sign of `leak`; an exponentially
collapsing bound; a linearly collapsing bound; the coherence
nonlinearity at two coherences, which is the pair that pins `alpha`;
uniform start-point variability; and a uniform lapse. For each of these,
three things: the defective density at both boundaries on a common time
grid, the two boundary masses, and the log-likelihood of a small fixed
dataset whose response times fall between grid nodes, so that the
likelihood’s own interpolation is exercised. At the shipped grid the two
agree to between 7e-4 and 1.6e-3 in the log density on every case but
one, which is an order of magnitude better than the 0.01 claimed above,
and the test also requires the disagreement to shrink as `dt` and `ny`
are refined, so the reference is what the solver converges to rather
than merely something it lands near.

**What the reference does not cover.** The leading edge: within a few
steps of zero both solvers spread a little mass where the true
first-passage density is exponentially small, for the same structural
reason, so the comparison starts at a decision time of 0.2 s and says
nothing below it. Anything outside those ten cases, in particular a
component you write yourself. Fitting: this compares densities, not
estimates. And the two packages differ in two conventions that are
translated rather than tested: this family renormalizes each condition’s
pair of densities so that its likelihood is conditional on a response
inside the window, while PyDDM keeps the undecided mass; and this family
spreads a lapse over `t_max` where PyDDM spreads it over one time step
more.

**Where they disagree.** On the exponentially collapsing bound the two
sit 2.8e-3 apart in the log density, and refining this package’s grid
alone does not close it. That is the reference’s error, not this
package’s, and the way to see it is to refine the REFERENCE. PyDDM has
no Crank-Nicolson path for a boundary that moves, so it solves that case
with backward Euler on a grid-snapped bound, first order in both steps,
and its own recorded convergence residual is 2.6e-3. Solved one level
finer and extrapolated to zero step, PyDDM moves toward this package:
measured against that limit rather than against the frozen fixture,
[`gddm()`](https://aforren1.github.io/frmtmb/frmtmb.eam/reference/gddm.md)
is 8.1e-4 out at the shipped grid, then 2.8e-4, then 1.7e-4 as its own
grid is refined. 8.1e-4 is inside the band the other nine cases occupy,
so there is no floor here and nothing about this case that
[`gddm()`](https://aforren1.github.io/frmtmb/frmtmb.eam/reference/gddm.md)
gets wrong.

The same is true of the start-variability case, which looks as though it
settles at 2.6e-4. `ICRange` lays the uniform start on whole grid cells,
so PyDDM’s box is about one cell too wide, an error first order in its
spatial step. Against PyDDM’s extrapolated limit
[`gddm()`](https://aforren1.github.io/frmtmb/frmtmb.eam/reference/gddm.md)
runs 7.2e-4, 2.1e-4, 6.5e-5 over the same three grids, which is the
ordinary second-order sequence. In both cases the mechanism is a
first-order error in the reference, and in both
[`gddm()`](https://aforren1.github.io/frmtmb/frmtmb.eam/reference/gddm.md)
is the more accurate of the two: holding the walls still, and spreading
the start with an integrated cubic B-spline instead of snapping it to
cells, is what buys that.

**A third opinion.** Both packages march a partial differential equation
on a grid, so agreeing with each other does not rule out an error they
make in the same direction. The test also simulates the stochastic
differential equation directly by Euler-Maruyama on the collapsing-bound
case, where a shared error would be least surprising. A simulated first
passage is late, because absorption is only noticed at a monitoring
time, so two monitoring gaps driven by the same Brownian path are
extrapolated to zero gap. It agrees with both packages to one or two
standard errors, and it resolves a shared bias down to roughly two parts
in a thousand. It is the size of those gaps that carries the conclusion
and not which side of the reference a run falls on: the two quantities
compared are anti-correlated by construction, so landing on opposite
sides is the null expectation rather than evidence, and the observed
sides change with the seed.
...
for discrete barrier options. *Mathematical Finance*, 7(4), 325-349. The
result behind the Euler-Maruyama extrapolation: a first passage noticed
only at a monitoring time is late by an amount of the order of the
square root of the monitoring gap, not of the gap.
