...
(cf. `brm()`/brms). Interfaces can change until v1.0.
...
brms-style [`bf()`](https://aforren1.github.io/frmtmb/reference/bf.md)
specification into an intermediate representation (IR), assembles design
matrices and covariance-structure blocks from data, and generates an
...
`frmtmb_formula` (output of
[`bf()`](https://aforren1.github.io/frmtmb/reference/bf.md), mirrors the
brms shape):
...
- [`custom_family()`](https://aforren1.github.io/frmtmb/reference/frmtmb_family.md)
  is the same constructor with a user R lpdf.
  [`check_custom_family()`](https://aforren1.github.io/frmtmb/reference/check_custom_family.md)
  tapes it on toy data and runs finite-difference gradient checks.
...
2.  `nlminb(obj$par, obj$fn, obj$gr)` with glmmTMB-style restarts.
    ...
    exposes optimizer choice and iteration limits.
...
class (`sigma`, `shape`, `phi`, …) is a density on that PARAMETER,
carried through its inverse link with that map’s log-Jacobian, and a
`class = "Intercept"` density is evaluated at the intercept at the mean
of the predictors, which is the one brms constrains.
[`set_prior()`](https://aforren1.github.io/frmtmb/reference/set_prior.md)
...
refused row of a table is named in one message. Attaching brms alongside
frmtmb masks [`bf()`](https://aforren1.github.io/frmtmb/reference/bf.md)
