# Lane wt-reunc: a copy of dev/release/run-check.ps1 on the two packages
# this lane touches, frmtmb and frmtmb.sample, with the lane library
# first on R_LIBS. Only the tree, the library path, the output directory
# and the package list differ.
#
...
$LOG  = "$ROOT/dev/reunc-log/check.log"
$OUT  = "C:/Users/adf44/source/r/checkout-reunc"

if (-not (Test-Path $R)) { throw "R missing: $R" }

if (-not $env:REUNC_LIB) { $env:REUNC_LIB = "C:/Users/adf44/source/r/reunc-lib" }
$env:R_LIBS = "$env:REUNC_LIB;C:/Users/adf44/source/r/rellib-r3;C:/Users/adf44/AppData/Local/R/win-library/4.6"
$env:NOT_CRAN = "true"
...
# A passing "checking tests ... OK" echoes no counts into the check
# log, so the only record of what ran inside the check is
# <pkg>.Rcheck/tests/*.Rout. Deleting the tree destroys it, and the
# tree has to be deleted on this machine because the disk is tight.
# Copy it out first, beside the log, so the counts survive.
$routs = Get-ChildItem -Path $OUT -Recurse -Filter "*.Rout" `
  -ErrorAction SilentlyContinue
foreach ($ro in $routs) {
  $pkg = ($ro.FullName -split "\\" |
    Where-Object { $_ -like "*.Rcheck" }) -replace "\.Rcheck$", ""
  $dest = "$ROOT/dev/reunc-log/check-" + $pkg + "-" + $ro.Name
  Copy-Item $ro.FullName -Destination $dest -Force
}
