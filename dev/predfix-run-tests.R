...
# Lane wt-predfix copy of dev/release/run-tests.R: PREDFIX_ARM=base reads
# the released build alone, anything else puts the lane library first.
# It also prints each failing expectation, so a log is its own evidence.
LIB <- if (Sys.getenv("PREDFIX_ARM", "lane") == "base") {
  "C:/Users/adf44/source/r/rellib-r3"
} else {
  c("C:/Users/adf44/source/r/predfix-lib", "C:/Users/adf44/source/r/rellib-r3")
}
...
cat("LIB", dirname(find.package(p)), format(packageVersion(p)), "\n")
  ...
  for (t in res) {
    for (x in t$results) {
      if (!inherits(x, "expectation_success") &&
          !inherits(x, "expectation_skip")) {
        cat(if (inherits(x, "expectation_warning")) "WARNING [" else
              "FAILED [", t$test, "] ",
            gsub("[\r\n]", " ", substr(conditionMessage(x), 1, 300)), "\n",
            sep = "")
      }
    }
  }
