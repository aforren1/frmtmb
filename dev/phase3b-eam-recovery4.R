...
#
# Usage: Rscript dev/phase3b-eam-recovery3.R <arm> <seed> [<seed> ...]
# The build check: the rerun harness writing to recov-final/, run on the
# FINAL build to show the arms fitted on earlier builds did not move.
# (Originally: the rerun harness, taking a LIST of seeds and writing
# to dev/phase3b-log/recov-rerun/, so a replicate that failed in the
# first pass is refitted without overwriting the record of the failure.
# The second harness: the first plus arm `contfix`, and the library
# read from P3B_LIB first, so that it can run against a build installed
# after the first harness started.
...
#         contfix   as cont, with contaminant_range = c(0.1, 5), the
#                   range the contaminants were drawn from
...
stopifnot(arm %in% c("cens", "cont", "contdef", "contfix", "collapse"))
dir.create("dev/phase3b-log/recov-final", showWarnings = FALSE, recursive = TRUE)
...
simulate_one <- function(seed) {
  ...
  d <- do.call(rbind, out)
  if (arm %in% c("cont", "contdef", "contfix")) {
...
for (seed in seeds) {
  ...
  res <- tryCatch({
    ...
    list(seed = seed, arm = arm, fixef = fe, se = se, confint = ci,
         ...
         n_cont = if (arm %in% c("cont", "contdef", "contfix")) attr(d, "n_cont") else 0L,
  ...
  saveRDS(res, sprintf("dev/phase3b-log/recov-final/%s-%d.rds", arm, seed))
