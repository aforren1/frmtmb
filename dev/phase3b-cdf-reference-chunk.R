...
#
# Chunked, lower-precision rerun: Rscript dev/phase3b-cdf-reference-chunk.R <bits> <chunk> <nchunks>
...
ref_surv_large <- function(t, v, a, w) {
  ...
  repeat {
    ...
    s <- s + term
    if (k > 5L && abs(term) < abs(s) * mp(2)^(-PREC)) break
    k <- k + 1L
...
gr$agree <- agree
utils::write.csv(gr[rows, ], sprintf("dev/phase3b-log/cdf-ref-%d-chunk%d.csv", PREC, CH), row.names = FALSE)
quit(save = "no")
