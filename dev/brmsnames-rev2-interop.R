## Reviewer RECHECK on the round 1 build: copy of dev/brmsnames-rev-interop.R
## writing its own cache files.
## Reviewer, claim 7: third-party readers on a fit, base against lane.
##   Rscript dev/brmsnames-rev2-interop.R base|lane|compare   data seed 3
...
out <- sprintf("dev/stan-cache/brmsnames-rev2-interop-%s.rds", arm)
...
if (arm != "compare") {
  ...
  b <- readRDS(sprintf("dev/stan-cache/brmsnames-rev2-interop-%s.rds", "base"))
  l <- readRDS(sprintf("dev/stan-cache/brmsnames-rev2-interop-%s.rds", "lane"))
  for (k in names(b)) {
