...
##
## Usage: Rscript dev/priorform-suite.R [pkgdir]
## Writes dev/priorform-suite-<pkg>.tsv and prints the comparison.
...
if (length(args) >= 2L) {
  only <- strsplit(args[2], " ", fixed = TRUE)[[1]]
  files <- files[files %in% only]
}
...
for (f in files) {
  ...
  out <- suppressWarnings(system2(
    rscript, c(file.path(root, "dev", "priorform-run1.R"),
               shQuote(pkgdir), shQuote(filt)),
    stdout = TRUE, stderr = TRUE))
  line <- grep("^PRIORFORM ", out, value = TRUE)
  if (!length(line)) {
...
utils::write.table(tab, file.path("dev", paste0("priorform-suite-", pkg,
                                                ".tsv")), sep = "\t",
