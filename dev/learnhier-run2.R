...
# RDS. This is `learnhier-run.R` plus the PER-LEARNER vectors, in a
# SEPARATE FILE rather than as an edit, because `Rscript` reads a script
# incrementally and editing one under a running replicate splices the
# file: that cost this lane a 44-minute fit once already.
#
# What it adds, and why. Test 1 of the `sd(ndt)` investigation is a
# WITHIN-replicate prediction: if the Laplace error at the hard edge of
# the Wiener density is what shrinks the component, the shrinkage must
# be worse for learners whose observed fastest response sits closest to
# their own true non-decision time. That needs each learner's margin and
# each learner's fitted deviation, and the original record keeps only
# the summaries. Everything below is additive, so a record written by
# either runner is read by the same summariser.
#
...
if (!rec$ok) {
  ...
  if (DESIGN %in% c("rlddm", "rlddm5")) {
    ...
    # THE PER-LEARNER VECTORS, for Test 1. `margin` is how far each
    # learner's observed floor sits above its own true non-decision
    # time, which is how sharply bounded that learner's conditional is;
    # `u_hat` is the fitted deviation and `u_true` the induced one the
    # draw realized, both on the scale the block is estimated on and
    # both centered, so a shrinkage is a comparison of like with like.
    ut <- attr(d, "dev_fitted")[, "ndt"]
    uh <- as.numeric(frmtmb::ranef(fit)[[1L]][, 4L])
    rec$per_learner <- list(
      margin = own - tru, floor = own, ndt_true = tru, ndt_hat = hat,
      u_true = ut - mean(ut), u_hat = uh - mean(uh),
      ranef = as.matrix(frmtmb::ranef(fit)[[1L]]),
      dev_true = attr(d, "dev_fitted"))
  }
