# Copy of phase3a-cure36.R that fits proportional hazards (no gamma1
# term) when PHASE3A_CURE_FORM=ph, for the review of punch round 2.
...
for (df in 3:6) {
  for (s in 20260924L + 0:19) {
    ...
    f <- tryCatch(suppressWarnings(frm(
      if (identical(Sys.getenv("PHASE3A_CURE_FORM"), "ph")) bf(t | cens(censored) ~ x) else
