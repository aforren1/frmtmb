...
source("C:/Users/adf44/source/r/frmtmb-wt-gddm/dev/rev-gddm-designs.R",
       local = new.env())
cat("\n\n== recorder over the reviewer's extra designs\n")
cat("  gddm frames accepted:", REC$frames,
    "  comparisons:", length(REC$rel), "\n")
nz <- REC$rel[REC$rel > 0]
cat("  exactly zero:", sum(REC$rel == 0), "  nonzero:", length(nz), "\n")
if (length(nz)) {
  w <- REC$where[REC$rel > 0]
  o <- order(nz, decreasing = TRUE)
  for (i in head(o, 8L)) cat(sprintf("    %-24s %.3e\n", w[i], nz[i]))
  for (cst in c(1e-9, 1e-11, 1e-12)) {
    cat(sprintf("    a constant of %.0e: %d of %d would flip\n", cst,
                sum(nz > cst), length(REC$rel)))
  }
}
