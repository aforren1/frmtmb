# Lane wt-reunc: a copy of dev/release/run-suite.ps1 whose only
# changes are the tree, the runner (dev/reunc-run-tests.R, the lane
# library first) and the log path. The release script is not edited.
#
...
$S    = "$ROOT/dev/reunc-run-tests.R"
$LOG  = "$ROOT/dev/reunc-log/suite.log"

if (-not (Test-Path $S)) { throw "runner missing: $S" }
if (-not (Test-Path $R)) { throw "Rscript missing: $R" }

$env:NOT_CRAN = "true"
# a later worker on this lane has a private library of its own, so an
# already-set REUNC_LIB wins over the default
