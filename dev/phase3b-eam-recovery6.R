...
# (Harness 6 is harness 5 writing to recov4/, run on the final build.)
...
# Writes dev/phase3b-log/recov4/<arm>-<seed>.rds, the fit or its error.
#
...
dir.create("dev/phase3b-log/recov4", showWarnings = FALSE, recursive = TRUE)
NS <- 30L; NT <- 400L
...
for (seed in seeds) {
  ...
  saveRDS(res, sprintf("dev/phase3b-log/recov4/%s-%d.rds", arm, seed))
