...
# Output: dev/phase3b-log/contwin/<design>-<seed>.rds
...
design <- a[[1]]
dir.create("dev/phase3b-log/contwin", showWarnings = FALSE)
for (seed in seq(as.integer(a[[2]]), as.integer(a[[3]]))) {
  ...
  saveRDS(list(seed = seed, design = design, truth = truth, n = nrow(d),
          ...
          sprintf("dev/phase3b-log/contwin/%s-%d.rds", design, seed))
  cat(design, seed, "\n")
}
