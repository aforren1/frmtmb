...
##
## Usage: Rscript dev/argspell-run1.R <pkgdir> <filter> [base]
...
##           library alone, which is 0.57.0, so a test can be seen
##           failing before the change is installed.
args <- commandArgs(trailingOnly = TRUE)
pkgdir <- args[1]
filt <- args[2]
base <- length(args) >= 3L
own <- if (base) character() else "C:/Users/adf44/source/r/argspell-lib"
.libPaths(c(own,
...
cat(sprintf("\nARGSPELL %s pass %d fail %d error %d skip %d in %.0f s\n",
