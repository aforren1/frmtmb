...
| 1.0b | Unblocked by 1.0a and unchanged in size. `rlddm()` can take `ndt_group()` as it stands, but needs an export seam from frmtmb.eam for four functions that are `@noRd` today; their shape depends on `rlddm()`, so 1.0a did not guess it. The floor table is keyed on the group LABEL rather than its level index, after review found that an index key pairs every row with another group's bound the moment a factor passes through `droplevels()`; the label key uses two rolling hashes packed below 2^49, whose collision residual is 1.8e-15 per pairing of an unseen label with a fitted group and is refused wherever both labels are visible. Keying on the label itself rather than a digest is filed for this item | `frmtmb.learn/R/rlddm.R` | the learn-rlddm scale row converges with a positive definite Hessian and no NaN standard errors | 0.5 |
...
| 1.0d | NEW, found by 1.0c. `frm_ode(n_ss =)` is a silent wrong answer during a fit. The steady-state run-in truncates at `exp(-n_ss * lambda_z * ii)`, where `lambda_z` is the SLOWEST disposition eigenvalue, so the shipped default of 20 is 3.9e-03 short on a fast two-compartment oral schedule, 1.2e-02 at a 107 h terminal half-life dosed daily and 2.9e-01 at 139 h dosed twice daily. On one 30-subject dataset simulated from the exact steady state it moves `k21` by a factor of 3.2 and `ke` by 11 percent. The `ss_tol` check cannot run on the tape at all, so a fit is never told, and the numeric path's warning reports the cycle-to-cycle movement, which understates the distance to the limit by about `1 / (lambda_z * ii)`, measured at 3.1x, 6.6x and 10.8x as the half-life grows, worst exactly where the error is worst. `?frm_ode` said "only a percent or two for one whose half-life is many intervals long", which was wrong by an order of magnitude and is corrected as of this round; what is left is the DEFAULT and the diagnostic. Raising the constant is the wrong lever and the review priced it: 50, 100 and 266 cost 7100, 12700 and 31400 s on the Phase 0 design while covering terminal half-lives of only 1.9, 3.8 and 10 dosing intervals, so no constant is both correct and affordable. Its proposal, unbuilt, is `n_ss = "auto"` read off the contraction ratio `ode_run_in()` already computes, which on the Phase 0 design picks 12 rather than 20 and is about 24 percent FASTER while becoming correct by construction; its stated unmeasured risk is how far that ratio moves between the starting values and the optimum | `frmtmb.ode/R/ode.R` | a two-compartment oral fit at a 107 h terminal half-life recovers `k21` inside its own interval at the shipped default, or the default is raised and the cost of raising it is stated | 1 |
...
Phase 1 total: about 11 days, of which 1.5 remain. Items 1.1 through
1.6 are done. Items 1.0a and 1.0c are done. What is left is 1.0b, half
a day once its export seam is settled, and 1.0d, one day, which 1.0c
found rather than inherited.

Rule 3 ordered the original three and it still orders what is left. Item
1.0a WAS the silent wrong answer: the `sv` arm of the eam row converged,
reported no warning, and gave a population non-decision time wrong by
ten percent with a standard error of 7.2e-06 on it, because the scaled
logit's derivative vanishes at the edge it had been pushed to. That arm
now converges to 0.2479 against a truth of 0.25 with 30 of 30 subjects
inside their own floor. Item 1.0d is the same shape and inherits the
priority: a fit is never told that its steady state has not converged,
and the bias is in the same direction in every group.

Two things this round settled that the plan should carry forward. The
0.55.1 disclosure in `frmtmb.eam/NEWS.md` telling users not to put a
random effect on `ndt` is discharged. And `gddm()` reads every dpar at
the first row of its condition, so a dpar that varies within a
condition is silently ignored; that is a new small item, not yet
numbered, and it is a silent wrong answer wider than the `ndt` case
that exposed it.
...
| 1, small and high leverage | 11, 1.5 left | 2 | none |
| 2, hierarchy validation | 7 | 1 | 2.1 unblocked by 1.0a; the 2.2 rlddm half still waits on 1.0b |
...
| total | about 55.5, with 1.5 of Phase 1 left | | |
