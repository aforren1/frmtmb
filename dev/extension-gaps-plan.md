...
| item | what | where | check | days |
|---|---|---|---|---|
| 2.5a | The mechanical half. For every generic with a canonical owner, stop defining a rival and register frmtmb's method onto the owner's generic instead, by delayed registration in `.onLoad` so that no new hard dependency is added: `posterior` owns `as_draws`, `as_draws_array`, `as_draws_df`, `as_draws_list`, `as_draws_matrix`, `as_draws_rvars`, `variables`, `ndraws`, `nchains`, `niterations`, `nvariables`; `loo` owns `loo`, `loo_compare`, `waic`; `bayesplot` owns `pp_check`; `rstantools` owns `bayes_R2` and `prior_summary`. These are meaningless without their owner, so "unavailable when the owner is absent" is the correct behavior rather than a regression. Vendor the public-domain `s3_register()` helper rather than taking a dependency on rlang. **Audit first which package defines each name**, because some of these are sampling concepts that may belong in frmtmb.sample rather than core | core `NAMESPACE`, `R/zzz.R`, and whichever package the audit says owns each | with brms attached FIRST and frmtmb second, every brms method on every listed generic still dispatches; and with the owner absent, frmtmb still loads | 1 |
...
| 2.5c | **OPEN QUESTION, not a task.** `fixef`, `ranef` and `VarCorr` have exactly one owner among the usual candidates, `lme4`, and `nlme` does not export them. frmtmb needs all three standalone, so "register conditionally and be absent otherwise" is not available. Importing lme4 puts Rcpp, RcppEigen, minqa, nloptr and boot behind a package whose Imports are currently eleven entries of base, Recommended and the RTMB family. The options, priced: import lme4 and accept the weight; keep the rival generic and accept that lme4 and brms users lose their methods; or keep the generic and add a `.default` that forwards to another loaded namespace's method, which works but is re-implementing dispatch. Decide before building 2.5a, because the answer may change its shape | core | a decision recorded with its reason | 0 |
...
Phase 2.5 total: about 3.5 days, plus one decision.
...
| 3.6 | spline: widen `rp_floored()` to see a group with NO events. A DEFECT rather than a feature, filed by item 2.5. `mono_rows <- which(f$cens == 0 & f$detadx <= 0)` in `R/rp-check.R` tests only uncensored rows, so for an all-censored group the score in its deviation is `-sum(x_i H_i) < 0` with no `log(gamma1 + u)` barrier to stop it, because the barrier lives in the density and that group contributes none. Measured: on 4 of 6 seeds five all-censored centres land at slopes of -0.21 to -0.31, and at one seed the fit converges at code 0 with a maximum gradient of 2.74e-05 and a positive definite Hessian, nothing warns, the check reports zero non-monotone rows and does not refuse, `frm_curve()` passes it through, and one centre's fitted survival RISES from 4.16e-50 to 0.774. The one-line widening is NOT the work: it changes a shipped refusal, and a check that fires on a correct model is worse than no check. It also raises a charter question worth settling first, since `rp_floored()` answers "is this fit's reported likelihood the model's" and `sp_floor_pos()` never touches a censored row, so the widened check answers a DIFFERENT question, "is the fitted hazard a hazard everywhere the data are", which may deserve its own name | `frmtmb.spline/R/rp-check.R` | a false-alarm rate on ordinary right-censored designs at the `df` values flexsurv users pick, not a patch; the inverse test added by item 2.5 must FLIP from passing to failing when this lands | 1 |
...
| `TapeConfig(vectorize = "enable")`, measured per family | filed, unmeasured | kaskr recommends it in the RTMB tips and the tmbstan vignette, where it gives a 2.3x speedup on a random regression and is applied before `MakeADFun`. frmtmb is close to the ideal case, since the objective is a vectorized RTMB closure. Two things stop it being a free switch. It is GLOBAL state, so anything setting it must restore it on exit. And a first probe here found it changing a Poisson GLMM's objective in the last bits, 1.54e-16 relative, while a gaussian GLMM and a wiener fit stayed bitwise identical: this repository pins bitwise identities in several places, including 0 ulp over 50 fixed-parameter probes, `identical()` on 26 of 26 ODE schedules and a Stan identity at 4.9e-15. That probe's TIMINGS were not trustworthy, because the tape accessor returned the wrong object and reported 1.3e-09 s per evaluation, so the speed question here is entirely open. The work is to measure it properly with `GetTape()$timer()` across the model families, check bitwise agreement on each, and decide per family rather than globally | | a speedup measured with an interleaved control, and a bitwise comparison per family; a decision recorded per family with its number | 1 |
...
| 2.5, do not break brms | 3.5 | nothing; it is a defect | 2.5c decides the shape of 2.5a |
