## Reviewer, claim 6: same numbers under the renaming. Run once per arm;
## each arm saves what its accessors return, then `compare` pairs them by
## POSITION and VALUE, never by name.
##   Rscript dev/brmsnames-rev-silent.R base
##   Rscript dev/brmsnames-rev-silent.R lane
##   Rscript dev/brmsnames-rev-silent.R compare
...
outf <- function(a) sprintf("dev/stan-cache/brmsnames-rev-silent-%s.rds", a)
