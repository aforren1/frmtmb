...
# (Harness 7: harness 6 taking a LIST of seeds and writing to recov5/,
# to refit sequentially the seeds that failed for want of memory.)
# trials on the build with B1, M1 and decisions (a) and (b).
#
# Usage: Rscript dev/phase3b-eam-recovery6.R <arm> <seed_from> <seed_to>
...
# Writes dev/phase3b-log/recov5/<arm>-<seed>.rds, the fit or its error.
#
...
dir.create("dev/phase3b-log/recov5", showWarnings = FALSE, recursive = TRUE)
NS <- 30L; NT <- 400L
...
for (seed in seeds) {
  ...
  saveRDS(res, sprintf("dev/phase3b-log/recov5/%s-%d.rds", arm, seed))
