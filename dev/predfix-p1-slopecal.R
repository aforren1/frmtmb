# Punch round 1, M1 calibration: at which column spread does a random
...
fo <- bf(y ~ x + (1 + x | g))
rows <- list()
for (fam in c("gau", "pois")) for (tau in c(0.1, 0.4)) for (seed in 1:8) {
  d <- mk(seed, tau, fam)
  fm <- if (fam == "gau") gaussian() else poisson()
  d$x <- d$x0
  ref <- max(ll(fit(fo, fm, d, FALSE)), ll(fit(fo, fm, d, TRUE)), na.rm = TRUE)
  for (s in c(1e-1, 3e-2, 1e-2, 5e-3, 3e-3, 2e-3, 1.5e-3)) {
    d$x <- d$x0 * s
...
saveRDS(r, "C:/Users/adf44/source/r/frmtmb-wt-predfix/dev/predfix-log/p1-slopecal.rds")
options(width = 200)
cat("autoscale = FALSE short by > 1e-6, by spread s (32 fits per s)\n")
...
print(r[r$short_false > 1e-6 & r$s >= 5e-3, ], digits = 6)
