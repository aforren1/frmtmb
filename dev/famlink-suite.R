...
##
## Usage: Rscript dev/famlink-suite.R [pkgdir]
## Writes dev/famlink-suite-<pkg>.tsv and prints the comparison.
...
for (f in files) {
  ...
  out <- suppressWarnings(system2(
    rscript, c(file.path(root, "dev", "famlink-run1.R"),
               shQuote(pkgdir), shQuote(filt)),
    stdout = TRUE, stderr = TRUE))
  line <- grep("^FAMLINK ", out, value = TRUE)
  if (!length(line)) {
...
utils::write.table(tab, file.path("dev", paste0("famlink-suite-", pkg, ".tsv")), sep = "\t",
