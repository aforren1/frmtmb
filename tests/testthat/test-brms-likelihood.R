...
test_that("row 22c: brms 2.23.0 cannot compile its own softit program", {
  ...
  # with a vector / vector division, which Stan rejects. stanc stops on
  # the FORWARD helper first, at the `softit(vector)` body; the inverse
  # link is wrong the same way one function later:
  #
  ...
  expect_true(grepl("log1p_exp(y) / (1 + log1p_exp(y))", code,
                    fixed = TRUE))
  # and neither body is the element-wise form Stan would accept. Scoped
  # to the two helper bodies: a bare "./" would also fire on an
  # unrelated elementwise division elsewhere in the program, and the
  # failure would then point at softit for no reason.
