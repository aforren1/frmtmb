...
test_that("blocks sharing a term label are all reported, not the first twice", {
  ...
  dd$y <- 1 + 0.5 * dd$x + u[as.integer(dd$id)] + pe[as.integer(dd$id)] +
    stats::rnorm(nrow(dd), 0, 0.5)
  # brms refuses the same coefficient on one group twice ("Duplicated
  # group-level effects are not allowed"), and so does frmtmb now
  expect_error(frm(bf(y ~ x + (1 | gr(id, cov = A)) + (1 | id)) +
                     gaussian(), data = dd, data2 = list(A = A)),
               "Duplicated group-level effects are not allowed")
  # two blocks still share the label "1 | id" when they sit on different
  # distributional parameters, which brms allows
  fit <- frm(bf(y ~ x + (1 | gr(id, cov = A)), sigma ~ (1 | id)) +
               gaussian(), data = dd, data2 = list(A = A))

  vc <- varcorr_matrices(fit)
  expect_length(vc, 2L)
  # the label of a block on sigma carries the dpar, so the two keys no
  # longer repeat; the checks below are that neither block is lost
  expect_identical(names(vc), c("1 | id", "sigma: 1 | id"))
