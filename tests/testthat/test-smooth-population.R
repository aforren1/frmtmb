...
# frm_linpred(gam, exclude = ). A smooth's wiggly part is a random-effect
...
test_that("re_formula = NA keeps the population smooth and drops the fs term", {
  ...
  expect_lt(max(abs(as.numeric(frm_linpred(fit, re_formula = NA)) - pop_gam)),
            1e-6)
  ...
  expect_lt(max(abs(
    as.numeric(frm_linpred(fit, newdata = nd, re_formula = NA)) -
...
test_that("an unseen fs level errors, and is allowed at the population level", {
  ...
  expect_equal(as.numeric(frm_linpred(fit, newdata = nd,
...
test_that("s(g, bs = 're') is a group-level smooth", {
  ...
  gre <- mgcv::gam(y ~ s(t, k = 6) + s(g, bs = "re"), data = d,
                   method = "ML")
  expect_lt(max(abs(as.numeric(frm_linpred(fre, re_formula = NA)) -
  ...
  expect_error(frm_linpred(fre, newdata = nd_new, allow_new_levels = TRUE),
               "no zero row")
  expect_length(frm_linpred(fre, newdata = nd_new, re_formula = NA), 2L)
})
